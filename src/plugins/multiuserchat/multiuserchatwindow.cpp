...
void MultiUserChatWindow::toolTipsForUser(IMultiUser *AUser, QMap<int,QString> &AToolTips)
{
	if (FUsers.contains(AUser))
	{
		AToolTips.insert(MUTTO_MUC_NICKNAME,QString("<big><b>%1</b></big>").arg(HTML_ESCAPE(AUser->nickName())));

		Jid realJid = AUser->data(MUDR_REAL_JID).toString();
		if (!realJid.isEmpty())
			AToolTips.insert(MUTTO_MUC_REALJID,tr("<b>Jabber ID:</b> %1").arg(HTML_ESCAPE(realJid.uBare())));
		...
		if (!role.isEmpty())
		{
			...
			AToolTips.insert(MUTTO_MUC_ROLE,tr("<b>Role:</b> %1").arg(HTML_ESCAPE(roleName)));
		}
		...
		if (!affiliation.isEmpty())
		{
			...
			if (affiliation == MUC_AFFIL_NONE)
				...
				affilName = tr("Owner");
			AToolTips.insert(MUTTO_MUC_AFFILIATION,tr("<b>Affiliation:</b> %1").arg(HTML_ESCAPE(affilName)));
		}
		...
		ttStatus = tr("<b>Status:</b> %1").arg(HTML_ESCAPE(statusName));
		if (!statusText.isEmpty())
			ttStatus +="<br>"+HTML_ESCAPE(statusText).replace('\n',"<br>");
...
void MultiUserChatWindow::fillPrivateChatContentOptions(IMessageChatWindow *AWindow, IMessageStyleContentOptions &AOptions) const
{
	...
	if (AOptions.direction == IMessageStyleContentOptions::DirectionIn)
	{
		...
		AOptions.senderName = HTML_ESCAPE(AWindow->contactJid().resource());
	}
	else
	{
		AOptions.senderColor = "red";
		AOptions.senderName = HTML_ESCAPE(FMultiChat->nickName());
	}
...
// *** <<< eyeCU <<< ***
				if(mouseEvent->button()==Options::node(OPV_MUC_GROUPCHAT_ADDRESSBUTTON).value().toInt() && userItem && (userItem->text()!=FMultiChat->nickName()))
// *** >>> eyeCU >>> ***
				{
...
// *** <<< eyeCU <<< ***
				...
				if (mouseEvent->button()==Qt::LeftButton && !userItem)
// *** >>> eyeCU >>> ***
				{
	...
	if (window)
	{
		if (FUsers.contains(AUser) || !FDestroyTimers.contains(window))
		{
			if (!enterMessage.isEmpty())
				showPrivateChatStatusMessage(window,enterMessage,AShow!=IPresence::Offline && AShow!=IPresence::Error ? IMessageStyleContentOptions::StatusJoined : IMessageStyleContentOptions::StatusLeft);
...
void MultiUserChatWindow::onUserDataChanged(IMultiUser *AUser, int ARole, const QVariant &ABefore, const QVariant &AAfter)
{
	if (ARole == MUDR_ROLE)
	{
		if (AAfter!=MUC_ROLE_NONE && ABefore!=MUC_ROLE_NONE)
			showMultiChatStatusMessage(tr("%1 role changed from %2 to %3").arg(AUser->nickName()).arg(ABefore.toString()).arg(AAfter.toString()),IMessageStyleContentOptions::TypeEvent);
		highlightUserRole(AUser);
	}
	else if (ARole == MUDR_AFFILIATION)
	{
		if (FUsers.contains(AUser))
			showMultiChatStatusMessage(tr("%1 affiliation changed from %2 to %3").arg(AUser->nickName()).arg(ABefore.toString()).arg(AAfter.toString()),IMessageStyleContentOptions::TypeEvent);
		highlightUserAffiliation(AUser);
	}
}
...
void MultiUserChatWindow::onUserNickChanged(IMultiUser *AUser, const QString &AOldNick, const QString &ANewNick)
{
	...
	showMultiChatStatusMessage(tr("%1 changed nick to %2").arg(AOldNick).arg(ANewNick),IMessageStyleContentOptions::TypeEvent);
}
