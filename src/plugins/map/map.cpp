...
Action *Map::addMenuAction(const QString &AText, const QString &AIcon, const QString &AKeyIcon, int AMenuTypes)
{
	...
	switch(AMenuTypes){
		...
		case MENU_ALL:	   FMenuToolbar->addAction(action, AG_MAPS_MENU_COMMON, false);
						   FMenuMap->addAction(action, AG_MAPS_MENU_COMMON, false);		break;
		case MENU_TOOLBAR: FMenuToolbar->addAction(action, AG_MAPS_MENU_COMMON, false);	break;
		default:		   FMenuMap->addAction(action, AG_MAPS_MENU_COMMON, false);
	}
	return action;
}
