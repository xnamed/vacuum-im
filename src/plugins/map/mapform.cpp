	...
	FWorkScaleFactor(1.0)
{
	...
	FSizePimap=16*IconStorage::scale();
#else
...
//----
	...
	connect(ui->rbtMode4, SIGNAL(clicked(bool)),this,SLOT(onTypeSelected(bool)));
...
void MapForm::setImage(QLabel *ALabel, int AType)
{
	...
	{
		case ICON_MAP:
			ALabel->setPixmap(FMap->getIcon(MPI_MAP).pixmap(FSizePimap));
			break;
		case ICON_MAP1:
			ALabel->setPixmap(FMap->getIcon(MPI_MAP1).pixmap(FSizePimap));
			break;
		case ICON_MAP2:
			ALabel->setPixmap(FMap->getIcon(MPI_MAP2).pixmap(FSizePimap));
			break;
		case ICON_SATELLITE:
			ALabel->setPixmap(FMap->getIcon(MPI_SATELLITE).pixmap(FSizePimap));
			break;
		case ICON_HYBRID:
			ALabel->setPixmap(FMap->getIcon(MPI_HYBRID).pixmap(FSizePimap));
			break;
		case ICON_TERRAIN:
			ALabel->setPixmap(FMap->getIcon(MPI_TERRAIN).pixmap(FSizePimap));
			break;
	}
}
