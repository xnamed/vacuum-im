...
// *** <<< eyeCU <<< ***
	...
	if (!isSilent && (record.notification.kinds & INotification::PopupWindow)>0)
	{
		if (!showNotifyByHandler(INotification::PopupWindow,notifyId,record.notification))
		{
            ...
            int sound = (record.notification.kinds & INotification::AndroidSound)	>0 ? 1 : 0;
            int vibro = (record.notification.kinds & INotification::AndroidVibrate)	>0 ? 2 : 0;
            int light = (record.notification.kinds & INotification::AndroidLights)	>0 ? 4 : 0;
